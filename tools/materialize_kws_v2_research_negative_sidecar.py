...
def main() -> int:
        ...
        split_rows[split].append(
            {
                ...
                "path": target.relative_to(split_dir).as_posix(),
    ...
    for split in SPLITS:
        ...
        receipts[split] = {
            ...
            "tsv": tsv.relative_to(output).as_posix(),
            "tsv_sha256": sha256_file(tsv),
            "references": refs.relative_to(output).as_posix(),
            "references_sha256": sha256_file(refs),
            "index": index_path.relative_to(output).as_posix(),
