...
def prepare(candidate: pathlib.Path, output: pathlib.Path) -> dict:
    ...
    if not isinstance(candidate_policy, dict):
        raise ValueError("RNN source development policy has no candidate_freeze")
    ...
    if not isinstance(arena, dict) or arena.get("model_family") != "rnn":
        raise ValueError(f"RNN shadow arena is not registered: {arena_name}")
    if arena.get("status") != "reserved-untouched":
        raise ValueError("RNN shadow arena is no longer reserved-untouched")

    candidate_model_sha256 = str(freeze["model_sha256"])
    for row in rows:
        if str(row.get("name", "")) == arena_name or row.get("model_family") != "rnn" or row.get("status") == "reserved-untouched":
            continue
        if str(row.get("candidate_model_sha256", "")) == candidate_model_sha256:
            raise ValueError("RNN model already consumed by an earlier shadow arena")

    seeds = [int(value) for value in arena.get("seeds", [])]
    if not 8 <= len(seeds) <= 16 or len(set(seeds)) != len(seeds):
        raise ValueError("RNN shadow arena seed contract is invalid")
    ...
    if formal in set(seeds) or retired & set(seeds):
        raise ValueError("RNN shadow arena overlaps formal qualification namespace")

    effective = copy.deepcopy(config)
    raw = effective.get("shadow_qualification")
    if not isinstance(raw, dict):
        raw = {}; effective["shadow_qualification"] = raw
    raw["enabled"] = True
    raw["seeds"] = seeds
    ...
    return {
        "schema_version": 1,
        "policy": "frozen-rnn-shadow-arena-binding-v1",
        "model_family": "rnn",
