...
def validate_eval(
    ...
    recordings = json_int(
        provenance.get("recordings"), "evaluation provenance recordings", 1
    )
    detections = json_int(
    ...
    measured_corpus = sha256_value(
        provenance.get("audio_corpus_sha256"),
        "evaluation provenance audio_corpus_sha256",
    )
    if measured_corpus != corpus_digest(audio_files):
        raise ValueError(
            "evaluation provenance audio corpus digest is internally inconsistent"
        )
    if (
        measured_corpus != actual_corpus.get("corpus_sha256")
        or audio_files != actual_corpus.get("recordings")
    ):
        raise ValueError(
            "evaluation provenance does not match selected qualification WAV bytes"
        )

    result = {
        "recordings": json_int(summary["recordings"], "evaluation.recordings", 1),
        "audio_hours": finite(
            summary["audio_hours"], "evaluation.audio_hours", 0.0
        ),
    ...
    if (
        result["audio_hours"] <= 0.0
        or result["recordings"] != recordings
        or result["recordings"] != len(audio_files)
    ):
    ...
    if result["matched"] + result["false_accepts"] != detections:
        raise ValueError(
            "evaluation matched/false-accept counts disagree with detections"
        )
    expected_frr = (
        result["false_rejects"] / result["expected"]
        if result["expected"]
        else 0.0
    )
    ...
    if (
        result["frr"] > 1.0
        or result["p50_post_end_latency_ms"]
        > result["p95_post_end_latency_ms"]
    ):
...
def validate_board(
    ...
    if (
        json_int(summary.get("block_samples"), "board.block_samples")
        != FRAME_HOP_SAMPLES
    ):
        raise ValueError("board benchmark must use one KWS hop per block")
    if (
        json_int(summary.get("model_bytes"), "board.model_bytes", 1)
        != model_bytes
        or json_int(
        ...
        raise ValueError(
            "board benchmark artifact sizes do not match selected artifacts"
        )
    result = {
        **actual_hashes,
        "audio_seconds": finite(
            summary["audio_seconds"], "board.audio_seconds", 0.0
        ),
        ...
        "p99_headroom": finite(
            summary["p99_headroom"], "board.p99_headroom", 0.0
        ),
    }
    if (
        result["audio_seconds"] <= 0.0
        or result["block_deadline_us"] != 20000.0
    ):
        raise ValueError("board benchmark audio/deadline is invalid")
    if not (
        result["p50_process_us"]
        <= result["p95_process_us"]
...
def validate_evidence(evidence: dict, collector_sha256: str) -> dict:
    ...
    if (
        required_text(evidence, "collector", "evidence")
        != "collect_target_evidence.py"
    ):
        raise ValueError(
            "target evidence must be produced by collect_target_evidence.py"
        )
    if (
        sha256_value(
            evidence.get("collector_sha256"), "evidence.collector_sha256"
        )
        != collector_sha256
    ):
        raise ValueError(
            "target evidence collector hash does not match retained collector"
        )
        ...
        normalized_raw.append(
            {
                ...
                "sha256": sha256_value(
                    item.get("sha256"),
                    f"evidence.raw_evidence[{index}].sha256",
                ),
                "bytes": json_int(
                    item.get("bytes"),
                    f"evidence.raw_evidence[{index}].bytes",
                    1,
                ),
            }
        )

    runtime_soak_name = required_text(
        evidence, "runtime_soak_name", "evidence"
    )
    runtime_soak_sha256 = sha256_value(
        evidence.get("runtime_soak_sha256"), "evidence.runtime_soak_sha256"
    )
    power_raw_name = required_text(evidence, "power_raw_name", "evidence")
    power_raw_sha256 = sha256_value(
        evidence.get("power_raw_sha256"), "evidence.power_raw_sha256"
    )
    raw_by_name = {item["name"]: item for item in normalized_raw}
    if (
        runtime_soak_name not in raw_by_name
        or raw_by_name[runtime_soak_name]["sha256"] != runtime_soak_sha256
    ):
        raise ValueError(
            "target evidence runtime soak identity is missing from raw evidence"
        )
    if (
        power_raw_name not in raw_by_name
        or raw_by_name[power_raw_name]["sha256"] != power_raw_sha256
    ):
        raise ValueError(
            "target evidence power raw identity is missing from raw evidence"
        )
    ...
    result = {
        ...
        "cpu_percent": finite(
            evidence["cpu_percent"], "evidence.cpu_percent", 0.0
        ),
        ...
        "calibration_id": required_text(
            evidence, "calibration_id", "evidence"
        ),
