...
def main() -> int:
    ...
    references_rows = load_jsonl(args.references)
    ...
    if (
        manifest.get("deployment_tag") != deployment_tag
        or intake.get("deployment_tag") != deployment_tag
        or afe.get("deployment_tag") != deployment_tag
    ):
        raise ValueError("deployment identity differs across policy/corpus/AFE")
    if (
        afe.get("afe", {}).get("backend") != "command"
        or afe.get("afe", {}).get("shipping_authority") is not True
    ):
        raise ValueError("final AFE evidence is not shipping-authoritative command backend")
    if (
        intake.get("raw_audio_hashes_verified") is not True
        or intake.get("pii_fields_rejected") is not True
    ):
    ...
    negative_hours = sum(
        float(references[name]["duration_s"]) for name in negative_recordings
    ) / 3600.0
    if negative_hours <= 0.0:
        raise ValueError("qualification has no post-AFE negative exposure")

    false_accepts_negative = [
        row
        for row in false_accepts
        if str(row.get("recording")) in negative_recordings
    ]
    false_accepts_positive = [
        row
        for row in false_accepts
    ...
    unknown_false_rejects = [
        key for key in false_reject_counts if key[0] not in recordings
    ]
    if unknown_false_rejects:
        raise ValueError("false-reject evidence references unknown recordings")

    confidence = float(policy["confidence_level"])
    expected_total = int(score["expected"])
    fr_total = int(score["false_rejects"])
    if fr_total != len(false_rejects):
        raise ValueError("score summary false-reject count differs from evidence rows")
    if int(score["false_accepts"]) != len(false_accepts):
        raise ValueError("score summary false-accept count differs from evidence rows")
    ...
    far_upper = poisson_rate_upper(
        len(false_accepts_negative), negative_hours, confidence
    )

    expected_by_keyword = Counter()
    for row in recordings.values():
        for event in row.get("expected", []):
            expected_by_keyword[int(event["keyword_id"])] += 1
    fr_by_keyword = Counter(int(row["keyword_id"]) for row in false_rejects)
    ...
    for keyword in (1, 2):
        ...
        per_keyword[str(keyword)] = {
            ...
            "frr_upper_bound": (
                wilson_upper(rejects, expected, confidence) if expected else 1.0
            ),
        }
    ...
    for rule in policy.get("critical_positive_slices", []):
        ...
        for row in recordings.values():
            ...
            keywords_in_recording: set[int] = set()
            for event in row["expected"]:
                keyword = int(event["keyword_id"])
                expected[keyword] += 1
                keywords_in_recording.add(keyword)
            for keyword in keywords_in_recording:
                rejects[keyword] += false_reject_counts[(recording, keyword)]
        item = {}
        for keyword in (1, 2):
            ...
            if missed > count:
                raise ValueError(
                    f"slice {rule['name']} has more false rejects than expected events"
                )
            item[str(keyword)] = {
                "expected": count,
    ...
    if negative_hours + 1e-12 < float(minimums["negative_audio_hours"]):
        failures.append("minimum-post-afe-negative-hours")
    ...
    if float(score["p95_post_end_latency_ms"]) > float(
        gates["max_p95_post_end_latency_ms"]
    ):
    ...
    result = {
        ...
        "evidence_sha256": {
            ...
            "references": sha256_file(args.references),
        ...
        "next_gate": (
            ...
            if not failures
            else "real-human-qualification-failed"
        ),
    }
