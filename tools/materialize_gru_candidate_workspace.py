...
def materialize(candidate: pathlib.Path, output: pathlib.Path) -> dict:
    ...
    evidence = load_object(candidate / "selection-evidence.json")
    if output.exists():
    ...
    selected_round = int(evidence["selected_round"])
    selected_frontend = str(evidence["selected_frontend"])
    record = {
        "round": selected_round,
        "frontend": selected_frontend,
        "candidate": 0,
        "score": float(evidence["selected_score"]),
        "model": str((best / "model.kwm").resolve()),
        ...
        "keywords": str((best / "keywords.tsv").resolve()),
        "pack": str((best / "keywords.kwk").resolve()),
        "calibration": evidence["calibration"],
        "calibration_domains": evidence["calibration_domains"],
        "test": evidence["test"],
        "test_domains": evidence["test_domains"],
    ...
    manifest = {
        "schema_version": 1,
        "evidence_class": "verified-frozen-gru-candidate-workspace",
        "policy": "frozen-gru-candidate-workspace-v1",
        "development_qualified": True,
        "records": [record],
        "candidate_selection": {
            "policy": "latest-strict-gate-passing-round",
            "eligible_rounds": [selected_round],
        ...
        "frozen_candidate": {
            "policy": str(freeze["policy"]),
            "model_sha256": str(freeze["model_sha256"]),
            "selection_evidence_sha256": str(freeze["selection_evidence_sha256"]),
            "development_wav_identities_sha256": str(
                freeze["development_wav_identities_sha256"]
            ),
...
def main() -> int:
    ...
    value = materialize(args.candidate, args.output)
    print(
        json.dumps(
            {
                "materialized": True,
                "selected_round": value["candidate_selection"]["selected_round"],
                "model_sha256": value["frozen_candidate"]["model_sha256"],
                "candidate_stage_feedback_allowed": False,
            },
            sort_keys=True,
        )
