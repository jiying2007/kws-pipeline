...
def validate_manifest(manifest: dict) -> dict:
    ...
    if board.get("runtime_source_revision") != source_sha:
        raise ValueError("manifest board runtime source does not match qualification source")
    validate_sha(board.get("runtime_config_digest"), "board.runtime_config_digest")
    if not all(
        isinstance(board.get(key), str) and board.get(key).strip()
        for key in ("runtime_version", "runtime_target")
    ):
        raise ValueError("manifest board runtime identity is incomplete")
