...
def verify_manifest(work: pathlib.Path, config_path: pathlib.Path, policy_path: pathlib.Path) -> dict:
    ...
    if manifest.get("policy") != loop.POLICY:
        raise ValueError("RNN development loop policy mismatch")
    if manifest.get("model_family") != "rnn":
        raise ValueError("RNN development model family mismatch")
    ...
    if manifest.get("selected_round") != selected_round:
        raise ValueError("RNN selected round mismatch")
    if manifest.get("selected_score") != selected_score:
        raise ValueError("RNN selected score mismatch")
