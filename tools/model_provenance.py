...
def validate_model_provenance(
    ...
    *,
    ...
    normalized_quantization: dict[str, dict] = {
    ...
    for matrix in ("in_proj", "rec_proj", "out_proj"):
        ...
        normalized_quantization[matrix] = {
            "scale": finite(
                stats.get("scale"), f"model provenance quantization.{matrix}.scale", 0.0
            ),
            ...
            "rmse": finite(
                stats.get("rmse"), f"model provenance quantization.{matrix}.rmse", 0.0
            ),
            "signal_rms": finite(
        ...
        if normalized_quantization[matrix]["scale"] <= 0.0:
