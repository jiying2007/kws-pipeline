...
def validate_model_provenance(
    ...
    *,
    ...
    if json_int(model.get("feature_dim"), "model provenance model.feature_dim", 1) != feature_dim:
        raise ValueError("model provenance feature_dim does not match .kwm")
    if json_int(model.get("hidden_dim"), "model provenance model.hidden_dim", 1) != hidden_dim:
        raise ValueError("model provenance hidden_dim does not match .kwm")
    if json_int(model.get("vocab_size"), "model provenance model.vocab_size", 2) != vocab_size:
    ...
    if training_tokens_hash != training_tokens_sha256:
        raise ValueError("model provenance training-token hash does not match selected training vocabulary")
    byte_identical = boolean(tokens.get("byte_identical_to_training"), "model provenance tokens.byte_identical_to_training")
    ...
    for index, item in enumerate(manifests):
        if not isinstance(item, dict):
            raise ValueError(f"model provenance training.manifests[{index}] must be an object")
        normalized_manifests.append(
            {
                "name": text(item.get("name"), f"model provenance training.manifests[{index}].name"),
    ...
    normalized_training = {
        ...
        "batch_size": json_int(training.get("batch_size"), "model provenance training.batch_size", 1),
        "learning_rate": finite(training.get("learning_rate"), "model provenance training.learning_rate", 0.0),
        "optimizer": text(training.get("optimizer"), "model provenance training.optimizer"),
        "weight_decay": finite(training.get("weight_decay"), "model provenance training.weight_decay", 0.0),
        "grad_clip_norm": finite(training.get("grad_clip_norm"), "model provenance training.grad_clip_norm", 0.0),
    }
    ...
    for matrix in ("in_proj", "rec_proj", "out_proj"):
        ...
        normalized_quantization[matrix] = {
            "scale": finite(stats.get("scale"), f"model provenance quantization.{matrix}.scale", 0.0),
            "max_abs_error": finite(stats.get("max_abs_error"), f"model provenance quantization.{matrix}.max_abs_error", 0.0),
            "rmse": finite(stats.get("rmse"), f"model provenance quantization.{matrix}.rmse", 0.0),
