...
def read_model(path: pathlib.Path) -> dict:
    ...
    if any(
        ...
        for scale in (wx_scale, wh_scale, wo_scale)
    ):
    ...
    for offset, count in ((bh_off, hidden_dim), (bo_off, model_vocab_size)):
        if any(
            not math.isfinite(struct.unpack_from("<f", blob, offset + i * 4)[0])
    ...
    return {
        ...
        "frontend_name": FRONTEND_KINDS[frontend_kind],
    }
...
def read_pack(path: pathlib.Path) -> dict:
    ...
    if not (
        1 <= count <= MAX_KEYWORDS and 2 <= pack_vocab_size <= MAX_VOCAB_SIZE
    ) or fingerprint == 0:
    ...
    for index in range(count):
        ...
        if any(token <= 0 or token >= pack_vocab_size for token in active) or any(
            tokens[num_tokens:]
        ):
...
def validate_runtime_config(path: pathlib.Path, model: dict) -> dict:
    ...
    if (
        json_int(config.get("frame_length_samples"), "config.frame_length_samples")
        != FRAME_LENGTH_SAMPLES
        or json_int(config.get("frame_hop_samples"), "config.frame_hop_samples")
        != FRAME_HOP_SAMPLES
    ):
        raise ValueError("runtime config frame geometry does not match ABI-v2")
    if (
        json_int(config.get("feature_dim"), "config.feature_dim", 1)
        != model["feature_dim"]
        or json_int(config.get("hidden_dim"), "config.hidden_dim", 1)
        != model["hidden_dim"]
    ):
