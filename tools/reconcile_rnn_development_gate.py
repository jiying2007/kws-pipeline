...
from rnn_development_gate import (  # noqa: E402
...
def _recreate_frozen_candidate(
    ...
    for path in (
        ROOT / "training" / "run_rnn_development.py",
        ROOT / "tools" / "rnn_development_gate.py",
...
def reconcile(
    ...
    if manifest.get("policy") != LOOP_POLICY:
        raise ValueError("unexpected RNN development loop policy")
    if manifest.get("model_family") not in (None, "rnn"):
        raise ValueError("unexpected RNN development model family")
    ...
    selected = loop.shared.select_best_strict_candidate(records) if stable else None
    qualified = stable and selected is not None

    manifest["model_family"] = "rnn"
