...
# configs/parameter-contract.json is the single source of truth for the
# per-keyword defaults and validation ranges.  src/keyword_pack.c reads the same
# file through the header generated by tools/gen_parameter_limits.py, so the
# compiler and the runtime cannot disagree about what a legal record is.
DEFAULT_CONTRACT = (
    pathlib.Path(__file__).resolve().parent.parent
    / "configs"
    / "parameter-contract.json"
)
WIRE_MAX = {"uint8": 0xFF, "uint32": 0xFFFFFFFF}


def load_contract(path: pathlib.Path) -> dict:
    if not path.is_file():
        raise RuntimeError(f"parameter contract not found: {path}")
    try:
        contract = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"{path}: not valid JSON: {exc}") from exc
    if not isinstance(contract.get("keyword_pack"), dict):
        raise RuntimeError(f"{path}: missing the 'keyword_pack' table")
    return contract


def contract_digest(path: pathlib.Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def bounded_int(raw: str, label: str, entry: dict) -> int:
    result = int(raw)
    low = entry.get("min")
    high = entry.get("max")
    if low is not None and result < low:
        raise ValueError(f"{label} must be >= {int(low)}")
    if high is not None and result > high:
        raise ValueError(f"{label} must be <= {int(high)}")
    wire = WIRE_MAX.get(entry.get("type"))
    if wire is not None and high is not None and int(high) > wire:
        raise ValueError(
            f"{label}: contract bound {int(high)} exceeds the on-wire "
            f"{entry.get('type')} field"
        )
    return result


def bounded_float(value: float, label: str, entry: dict) -> float:
    ...
    if low is not None and (
        ...
        operator = ">" if entry.get("min_exclusive") else ">="
        raise ValueError(f"{label} must be {operator} {low}")
    if high is not None and (
        value > high or (entry.get("max_exclusive") and value == high)
    ):
        operator = "<" if entry.get("max_exclusive") else "<="
        raise ValueError(f"{label} must be {operator} {high}")
    return value


def declared_default(entry: dict, label: str) -> int:
    value = entry.get("default")
    if value is None:
        raise RuntimeError(f"{label}: the parameter contract declares no default")
    return int(value)
...
def parse_keywords(
    path: pathlib.Path, token_map: dict[str, int], contract: dict
) -> list[dict]:
    ...
    for n, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        threshold = bounded_float(
            float(cols[2]), f"{text}: threshold", pack["threshold"]
        )
        tokens = (
            cols[3].split()
        ...
        min_trailing_blanks = (
            bounded_int(
                cols[4],
                f"{text}: min_trailing_blanks",
                pack["min_trailing_blanks"],
            )
            if len(cols) >= 5 and cols[4].strip()
            else declared_default(pack["min_trailing_blanks"], "min_trailing_blanks")
        )
        priority = (
            bounded_int(cols[5], f"{text}: priority", pack["priority"])
            if len(cols) >= 6 and cols[5].strip()
            else declared_default(pack["priority"], "priority")
        )
        policy_name = (
            cols[6].strip().lower()
        ...
        if policy_name not in PREFIX_POLICIES:
            raise ValueError(
                f"{text}: prefix_policy must be immediate, longest or grace"
            )
        grace_frames = (
            bounded_int(cols[7], f"{text}: grace_frames", pack["grace_frames"])
            if len(cols) >= 8 and cols[7].strip()
            else declared_default(pack["grace_frames"], "grace_frames")
        )
        prefix_policy = PREFIX_POLICIES[policy_name]
        if prefix_policy == PREFIX_POLICIES["longest"] and min_trailing_blanks == 0:
            min_trailing_blanks = int(
                policy_defaults["longest"]["min_trailing_blanks"]
            )
        if prefix_policy == PREFIX_POLICIES["grace"] and grace_frames == 0:
            grace_frames = int(policy_defaults["grace"]["grace_frames"])
        if kid in seen_ids:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="parameter contract that defines the keyword defaults and ranges",
    )
    ...
    items = parse_keywords(args.keywords, token_map, contract)
    fingerprint = vocab_fingerprint(token_map)
    if args.out_header:
            ...
            json.dumps(
                {
                    ...
                    "parameter_contract": contract.get("contract_id"),
                    "parameter_contract_sha256": contract_digest(args.contract),
                    "keywords": items,
                },
                ensure_ascii=False,
                indent=2,
