...
def main() -> int:
    ...
    expected_contract = config["required_cpu_contract"]
    records = {}
    for identity_path in args.root.resolve().rglob("identity.json"):
        ...
        if str(result["frontend"]) != str(spec["frontend"]):
            raise ValueError(f"{key}: frontend drift")
        ...
        if str(result["lr_schedule"]) != str(config["fixed"]["lr_schedule"]):
            raise ValueError(f"{key}: LR schedule drift")
        if str(result["init_mode"]) != str(config["fixed"]["init_mode"]):
            raise ValueError(f"{key}: init mode drift")
        if result["research_cpu_contract"] != expected_contract:
            raise ValueError(f"{key}: research CPU contract drift")
        records[key] = {
    ...
    if set(records) != expected:
        raise ValueError(
            f"record mismatch: have={sorted(records)} expected={sorted(expected)}"
        )
    ...
    for candidate in config["candidates"]:
        for raw_seed in config["fixed"]["model_seeds"]:
            seed = int(raw_seed)
            replicas = [
                records[(candidate, seed, replica)]
                for replica in config["fixed"]["replicas"]
            ]
            checks = {
                "initial_state_bit_exact": len(
                    {row["initial_model_state_sha256"] for row in replicas}
                ) == 1,
                "final_state_bit_exact": len(
                    {row["final_model_state_sha256"] for row in replicas}
                ) == 1,
                "history_bit_exact": all(
                    row["history"] == replicas[0]["history"] for row in replicas[1:]
                ),
                "calibration_bit_exact": all(
                    row["calibration"] == replicas[0]["calibration"]
                    for row in replicas[1:]
                ),
                "test_bit_exact": all(
                    row["test"] == replicas[0]["test"] for row in replicas[1:]
                ),
            }
            ...
            groups[f"{candidate}:{seed}"] = {
                "pass": group_pass,
                "checks": checks,
                "initial_model_state_sha256": replicas[0][
    ...
    next_experiment = config["decision_rules"][
        "pass_next" if all_pass else "fail_next"
    ]
    result = {
        "schema_version": 1,
        "evidence_class": "kws-v2-classifier-cross-runner-repro-summary-v1",
        "evidence_scope": "research-only",
        "diagnostic_only": True,
        "promotion_allowed": False,
        "protected_evidence_used": False,
        "shipping_metric": False,
        "reproducibility_pass": all_pass,
        "groups": groups,
