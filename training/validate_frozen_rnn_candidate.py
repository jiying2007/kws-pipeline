...
def validate(candidate: pathlib.Path, runner: pathlib.Path, output: pathlib.Path) -> dict:
    ...
    rnn_formal_seed = int(freeze_policy.get("formal_qualification_seed", -1))
    if namespace <= 0 or rnn_formal_seed <= 0:
        raise ValueError("RNN Fresh/Formal namespace is not frozen")
    ...
    sibling_gru_formal = int(config.get("qualification_holdout_seed", -1))
    protected = {sibling_gru_formal, rnn_formal_seed}
    ...
    if rnn_formal_seed == sibling_gru_formal:
        raise ValueError("RNN formal seed overlaps GRU formal seed")
    ...
    for split in ("calibration", "test", "qualification"):
        base, domains = evaluate(runner=runner, model=model, pack=pack, references=dataset / f"{split}.references.jsonl", output=output / f"eval-{split}")
        qualified = base_gate(base, gates) and domain_gate(domains, gates)
        all_qualified = all_qualified and qualified
        split_rows[split] = {"qualified": qualified, "metrics": base, "domains": domains, "references_sha256": sha256_file(dataset / f"{split}.references.jsonl")}

    result = {
        "schema_version": 1, "policy": POLICY, "model_family": "rnn", "qualified": all_qualified,
        "frozen_candidate_policy": str(freeze["policy"]), "model_sha256": candidate_model_sha256,
        "pack_sha256": str(freeze["pack_sha256"]), "fresh_validation_registry_entry": str(fresh_entry.get("name", "")),
        "fresh_validation_registry_status": str(fresh_entry["status"]), "fresh_validation_seed": seed,
        "fresh_validation_seed_namespace": namespace, "rnn_formal_seed_reserved": rnn_formal_seed,
        "gru_formal_seed_protected": sibling_gru_formal, "fresh_validation_wav_count": len(fresh_hashes),
        "fresh_validation_wav_sha256": sorted(fresh_hashes), "development_overlap_count": 0,
        "thresholds_recalibrated": False, "training_performed": False, "validation_feedback_allowed": False,
        "threshold_feedback_allowed": False, "training_rule_feedback_allowed": False, "formal_qualification_used": False,
        "shadow_used": False, "domain_index_sha256": str(summary["domain_index_sha256"]), "splits": split_rows,
    }
    write_object(output / "fresh-validation-summary.json", result); return result


def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--candidate", required=True, type=pathlib.Path); parser.add_argument("--runner", required=True, type=pathlib.Path); parser.add_argument("--output", required=True, type=pathlib.Path)
