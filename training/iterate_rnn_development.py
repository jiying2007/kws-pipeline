...
def _copy_frozen_candidate(selected: dict, output: pathlib.Path, config: pathlib.Path, policy: pathlib.Path) -> dict:
    ...
    code_paths = [pathlib.Path(__file__).resolve(), TRAINING / "train_ctc.py", TRAINING / "model.py", TRAINING / "export_model.py", TRAINING / "domain_curriculum.py", TRAINING / "hard_negative_replay.py", TRAINING / "development_failure_replay.py", TOOLS / "rnn_development_gate.py"]
