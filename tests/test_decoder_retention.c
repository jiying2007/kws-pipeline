...
static void verify_boundary_reset_preserves_short_pause_and_clears_long_gap(void) {
  kws_decoder_t decoder;
  const uint16_t tokens[] = {1u, 2u, 3u, 2u};
  kws_keyword_t item = {0};
  float logits[4];
  uint32_t keyword_id = 0u;
  float confidence = 0.0f;
  ...
  item.tokens = tokens;
  item.num_tokens = 4u;
  item.threshold = 0.10f;
  item.prefix_policy = (uint8_t)KWS_PREFIX_IMMEDIATE;
  kws_decoder_init(&decoder, 0.0f, 0.94f);
  CHECK(kws_decoder_set_keywords(&decoder, &item, 1u, 4u) == KWS_OK);

  /* The selected boundary policy is 13 x 20-ms inactive frames. A 12-frame
   * within-utterance pause must keep the exact half-keyword prefix alive. */
  set_logits(logits, -8.0f, 8.0f, -8.0f, -8.0f);
  CHECK(kws_decoder_step(&decoder, logits, 4u, 1, &keyword_id, &confidence) == 0);
  set_logits(logits, -8.0f, -8.0f, 8.0f, -8.0f);
  CHECK(kws_decoder_step(&decoder, logits, 4u, 1, &keyword_id, &confidence) == 0);
  ...
  /* The boundary reset must not poison the next complete utterance. */
  set_logits(logits, -8.0f, 8.0f, -8.0f, -8.0f);
  CHECK(kws_decoder_step(&decoder, logits, 4u, 1, &keyword_id, &confidence) == 0);
  set_logits(logits, -8.0f, -8.0f, 8.0f, -8.0f);
  CHECK(kws_decoder_step(&decoder, logits, 4u, 1, &keyword_id, &confidence) == 0);
...
int main(void) {
  ...
  verify_boundary_reset_preserves_short_pause_and_clears_long_gap();
