...
from direct_keyword_benchmark import POLICY,RUNTIME_ACCEPTANCE_THRESHOLD,ctc_sequence_loss,direct_class,frame_loss,grounded_ctc_loss,runtime_acceptance_loss,same_voice_pairs,write_vocab
...
class DirectKeywordBenchmarkTests(unittest.TestCase):
    ...
    def test_end_window_is_explicit_and_bounded(self):
        self.assertEqual(POLICY,'direct-whole-keyword-runtime-acceptance-paired-v5')
    ...
    def test_runtime_acceptance_loss_matches_fixed_threshold_and_ignores_negatives(self):
        logits=torch.full((5,2,3),-4.0)
        logits[:,:,0]=0.0
        logits[2,0,0]=-4.0;logits[2,0,1]=4.0
        lp=logits.log_softmax(-1)
        self.assertEqual(RUNTIME_ACCEPTANCE_THRESHOLD,0.55)
        self.assertAlmostEqual(float(runtime_acceptance_loss(lp,[5,5],[(1,4),(1,4)],[1,0])),0.0,places=6)
        logits[2,0,0]=0.0;logits[2,0,1]=0.0;lp=logits.log_softmax(-1)
        expected=max(0.0,__import__('math').log(0.55)-float(lp[2,0,1]))
        self.assertAlmostEqual(float(runtime_acceptance_loss(lp,[5,5],[(1,4),(1,4)],[1,0])),expected,places=5)

    def test_runtime_acceptance_geometry_fails_closed(self):
        lp=torch.randn(6,1,3).log_softmax(-1)
        for lengths,spans,classes in (([6],[(4,4)],[1]),([7],[(1,5)],[1]),([6],[(1,5)],[True]),([6],[(1,7)],[1])):
            with self.subTest(lengths=lengths,spans=spans,classes=classes), self.assertRaises(ValueError):
                runtime_acceptance_loss(lp,lengths,spans,classes)
