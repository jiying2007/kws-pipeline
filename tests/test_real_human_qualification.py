...
def write_wav(path: pathlib.Path, marker: int, seconds: float = 1.0) -> None:
    ...
    with wave.open(str(path), "wb") as writer:
        ...
        writer.writeframes(b"".join(struct.pack("<h", value * 32) for value in samples))
...
def main() -> int:
    production_policy = json.loads((ROOT / "commercial/real-human-qualification.policy.json").read_text(encoding="utf-8"))
    ...
    assert {row["name"] for row in production_policy["critical_positive_slices"]} >= {"distance_3_5m", "rear", "playback", "double_talk", "robot_motion"}

    workflow = (ROOT / ".github/workflows/real-human-qualification.yml").read_text(encoding="utf-8")
    assert "workflow_dispatch:" in workflow
    assert "runs-on: [self-hosted, kws-real-audio]" in workflow
    assert "Consume fresh corpus before KWS scoring" in workflow
    assert workflow.index("Consume fresh corpus before KWS scoring") < workflow.index("Run frozen KWS over post-AFE human audio")
    assert "human-exposure-${corpus_sha:0:16}" in workflow
    assert "raw audio remains restricted" in workflow.lower()
    assert "build/real-human/afe/post-afe" not in workflow.split("Retain qualification evidence without raw audio", 1)[1].split("Publish successful Phase-A receipt", 1)[0]
    ...
    with tempfile.TemporaryDirectory(prefix="kws-real-human-contract-") as tmp_raw:
        ...
        rows = [
            ("k1", "k1.wav", "spk-a", "s1", [{"keyword_id": 1, "start_s": 0.10, "end_s": 0.30}], ["household"]),
            ("k2", "k2.wav", "spk-a", "s1", [{"keyword_id": 2, "start_s": 0.10, "end_s": 0.30}], ["playback"]),
            ("neg", "neg.wav", "ambient-a", "n1", [], ["household", "speech_confusion", "playback", "robot_motion"]),
        ]
            ...
            manifest["recordings"].append(
                {
                    ...
                    "tags": tags,
                    "expected": expected,
                    "capture": {"sample_rate_hz": 16000, "channels": 1, "sample_format": "pcm_s16le"},
        ...
        policy = {
            ...
            "minimums": {"speakers": 1, "sessions_per_speaker": 1, "expected_wakes_total": 2, "expected_wakes_per_keyword": 1, "negative_audio_hours": 1.0 / 3600.0},
            "gates": {"max_frr": 0.01, "max_frr_upper_bound": 0.90, "max_far_per_hour": 0.01, "max_far_upper_bound_per_hour": 20000.0, "max_p95_post_end_latency_ms": 500.0},
        ...
        run("python3", "tools/validate_real_human_corpus.py", "--manifest", str(manifest_path), "--audio-root", str(audio_root), "--policy", str(policy_path), "--public-summary", str(intake))
        ...
        run("python3", "tools/validate_real_human_corpus.py", "--manifest", str(bad_path), "--audio-root", str(audio_root), "--policy", str(policy_path), "--public-summary", str(tmp / "bad.json"), expect=2)
        ...
        run("python3", "tools/run_final_afe_corpus.py", "--manifest", str(manifest_path), "--audio-root", str(audio_root), "--adapter", str(adapter_path), "--output-dir", str(afe_root))
        afe_summary = json.loads((afe_root / "afe-corpus-summary.json").read_text(encoding="utf-8"))
        ...
        detections.write_text(
            json.dumps({"recording": "k1", "keyword_id": 1, "time_s": 0.30, "confidence": 0.9}) + "\n" +
            json.dumps({"recording": "k2", "keyword_id": 2, "time_s": 0.30, "confidence": 0.9}) + "\n",
            encoding="utf-8",
        )
        score_summary = tmp / "score.json"
        false_accepts = tmp / "fa.jsonl"
        false_rejects = tmp / "fr.jsonl"
        run("python3", "eval/score_events.py", "--references", str(afe_root / "references.post-afe.jsonl"), "--detections", str(detections), "--summary", str(score_summary), "--false-positives", str(false_accepts), "--false-rejects", str(false_rejects))
        final = tmp / "qualification.json"
        run("python3", "tools/score_real_human_qualification.py", "--manifest", str(manifest_path), "--policy", str(policy_path), "--intake-summary", str(intake), "--afe-summary", str(afe_root / "afe-corpus-summary.json"), "--score-summary", str(score_summary), "--false-accepts", str(false_accepts), "--false-rejects", str(false_rejects), "--detections", str(detections), "--output", str(final))
