...
def main() -> int:
    ...
    gru_rows = [row for row in reference.get("rows", []) if row.get("arm") == "gru-64"]
    if len(gru_rows) != 1:
        raise ValueError("exactly one gru-64 reference row is required")
    gru = dict(gru_rows[0])
    gru["candidate"] = "gru-64-reference"
    gru["source"] = "model-family-ab-6"
    rows.append(gru)
    ...
    result = {
        "schema_version": 1,
        "policy": "focused-rnn-tuning-comparison-v1",
