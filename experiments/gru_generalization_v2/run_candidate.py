...
POLICY = "shadow-blind-gru-generalization-v2"
...
def main() -> int:
    ...
    extras = [pathlib.Path(v["manifest"]) for v in shared.get("train_only_seed_manifests", [])]
    if len(extras) != 2:
        raise ValueError("GRU V2 requires exactly two predeclared training-only acoustic manifests")
    for row, path in zip(shared["train_only_seed_manifests"], extras):
    ...
    base_manifests = [canonical, *extras]
    ...
    for path in manifests:
        if not path.is_file() or path.stat().st_size == 0:
            raise ValueError(f"missing GRU V2 manifest: {path}")
    ...
    manifest = {
        "schema_version": 1,
        "evidence_class": "development-only-shadow-blind-gru-generalization",
        "policy": POLICY,
